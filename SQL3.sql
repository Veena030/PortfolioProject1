...
SELECT Date,SUM(CAST(new_cases AS FLOAT)) AS totalcases, SUM(CAST(new_deaths AS FLOAT)) as TotalDeaths,(SUM(new_deaths)/SUM(new_cases))*100 AS NewDeathPercentage
FROM coviddeaths
WHERE COntinent <> ''
GROUP BY Date
ORDER BY date;

SELECT SUM(CAST(new_cases AS FLOAT)) AS totalcases, SUM(CAST(new_deaths AS FLOAT)) as TotalDeaths,(SUM(new_deaths)/SUM(new_cases))*100 AS NewDeathPercentage
FROM coviddeaths
WHERE COntinent <> ''
ORDER BY date;
